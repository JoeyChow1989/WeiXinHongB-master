...
public class QQHongbaoService extends AccessibilityService
{
    ...
    public void recycle1(AccessibilityNodeInfo info)
    {
        if (info.getChildCount() == 0)
        {
            if (info.getClassName().toString().equals("android.widget.EditText"))
            {
                ...
                arguments.putCharSequence(AccessibilityNodeInfo.ACTION_ARGUMENT_SET_TEXT_CHARSEQUENCE, MainActivity.qianshu + "    " + sp.getString("autothanks", "谢谢"));
    ...
    public void recycle2(AccessibilityNodeInfo info)
    {
        if (info.getChildCount() == 0)
        {
            ...
            {
                Bundle arguments = new Bundle();
                arguments.putCharSequence(AccessibilityNodeInfo.ACTION_ARGUMENT_SET_TEXT_CHARSEQUENCE, sp.getString("autothanks", "谢谢"));
                info.performAction(AccessibilityNodeInfo.ACTION_SET_TEXT, arguments);
                MainActivity.qianshu = null;
            }
    ...
    public void recycle3(AccessibilityNodeInfo info)
    {
        if (info.getChildCount() == 0)
        {
            ...
            {
                Bundle arguments = new Bundle();
                arguments.putCharSequence(AccessibilityNodeInfo.ACTION_ARGUMENT_SET_TEXT_CHARSEQUENCE, "@" + user + "   " + sp.getString("autothanks", "谢谢"));
                info.performAction(AccessibilityNodeInfo.ACTION_SET_TEXT, arguments);
                user = null;
            }
    ...
    public void onAccessibilityEvent(AccessibilityEvent event)
    {
        ...
        if (rootNodeInfo.getText() != null && rootNodeInfo.getPackageName().equals("com.tencent.mm"))
        {
            if (rootNodeInfo.getText().toString().contains(".") && rootNodeInfo.getText().toString().length() == 4)
            {
                ...
                editor.putString("laiyuan", "微信");
                editor.commit();
                huifu_weixin = true;
                huifu_mm = true;

                //System.out.println("---------存入---------:" + sp.getString("laiyuan", "") + MainActivity.qianshu + sp.getString("username", ""));
            }
        }

        //获取抢到的钱数(QQ);
        if (rootNodeInfo.getText() != null && rootNodeInfo.getPackageName().equals("com.tencent.mobileqq"))
        {
            if (rootNodeInfo.getText().toString().contains(".") && rootNodeInfo.getText().toString().length() == 5 && isCun == false)
            {
                MainActivity.qianshu = rootNodeInfo.getText().toString();
                editor.putString("laiyuan", "QQ");
                editor.commit();
                huifu_QQ = true;
                huifu = true;
                aite = true;

                User user = new User(sp.getString("laiyuan", ""), sp.getString("username", ""), MainActivity.qianshu, HbApplication.getInstance().getDate());
                HbApplication.getInstance().getUserDB().addUser(user);
                isCun = true;
                System.out.println("---------存入---------:" + sp.getString("laiyuan", "") + MainActivity.qianshu + sp.getString("username", ""));
